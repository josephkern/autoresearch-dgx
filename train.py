...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # SDPA expects (B, n_head, T, head_dim); window_size accepted but ignored on this kernel
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
# Optimization
TOTAL_BATCH_SIZE = 2**16 # ~65K tokens per optimizer step (DGX Spark / GB10)
...
steady_state_mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / GB10_BF16_PEAK_FLOPS if total_training_time > 0 else 0
